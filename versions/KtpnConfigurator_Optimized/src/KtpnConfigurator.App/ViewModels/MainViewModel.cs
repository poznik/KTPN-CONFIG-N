...
public sealed partial class MainViewModel : ObservableObject
{
    ...
    {
        _env = env;
        _equipmentDatabaseAudit = EquipmentDatabaseAudit.Analyze(env.Catalog);
        ...
        TtAccuracyClasses = env.Catalog.Options.TtAccuracyClasses;
        ...
        ApplyElectricalSelectionCommand = new RelayCommand(ApplyElectricalSelection);
        RestoreRecoveryCommand = new RelayCommand(RestoreRecovery, () => HasRecoveryProject);
    ...
    private ProjectConfig CreateDefaultConfig()
    {
        ...
        var cfg = new ProjectConfig
        {
            ...
            HasCtKip = false, CtKipRatio = "600/5", CtKipAccuracyClass = "0,5", HasMeter = true,
    ...
    public IReadOnlyList<int> PvrNominals => ElectricalSelectionService.InputNominals("ПВР/NH", _cfg.PvrManufacturer, _env.Catalog);
    public IReadOnlyList<int> ReNominals => ElectricalSelectionService.InputNominals("РЕ", _cfg.ReManufacturer, _env.Catalog);
    public IReadOnlyList<int> AvInNominals => ElectricalSelectionService.InputNominals("АВ", _cfg.AvInManufacturer, _env.Catalog);
    public IReadOnlyList<string> AvInManufacturers => ElectricalSelectionService.InputManufacturers("АВ", _env.Catalog);
    public IReadOnlyList<string> TtRatios { get; }
    public IReadOnlyList<string> TtAccuracyClasses { get; }
    ...
    // ===== Meta =====
    public string ProjectName { get => _cfg.ProjectName; set { if (_cfg.ProjectName != value) { _cfg.ProjectName = value; OnPropertyChanged(); Recalculate(); } } }
    public string Author { get => _cfg.Author; set { if (_cfg.Author != value) { _cfg.Author = value; OnPropertyChanged(); Recalculate(); } } }
    public bool AutoElectricalSelection
    {
        get => _cfg.AutoElectricalSelection;
        set
        {
            if (_cfg.AutoElectricalSelection == value)
                return;
            _cfg.AutoElectricalSelection = value;
            OnPropertyChanged();
            Recalculate();
        }
    }
    public string ProjectVersionText => $"Версия проекта: {_cfg.Revision}";
    public string ProjectVersionNumberText => _cfg.Revision.ToString(CultureInfo.InvariantCulture);
    ...
    // ===== РУНН ввод =====
    public bool PvrOn { get => _cfg.PvrOn; set { if (_cfg.PvrOn != value) { _cfg.PvrOn = value; OnPropertyChanged(); OnPropertyChanged(nameof(ShowNhRiseWarning)); Recalculate(); } } }
    public bool ShowNhRiseWarning => _cfg.PvrOn && !Aux.HasRise;
    public int PvrNominal { get => _cfg.PvrNominal; set { if (_cfg.PvrNominal != value) { DisableAutoSelectionForManualEdit(); _cfg.PvrNominal = value; OnPropertyChanged(); EnsureInputMeterCt(forceRatio: true); Recalculate(); } } }
    public string PvrManufacturer { get => _cfg.PvrManufacturer; set { if (_cfg.PvrManufacturer != value) { _cfg.PvrManufacturer = value; OnPropertyChanged(); OnPropertyChanged(nameof(PvrNominals)); Recalculate(); } } }
    public bool ReOn { get => _cfg.ReOn; set { if (_cfg.ReOn != value) { _cfg.ReOn = value; OnPropertyChanged(); Recalculate(); } } }
    public int ReNominal { get => _cfg.ReNominal; set { if (_cfg.ReNominal != value) { DisableAutoSelectionForManualEdit(); _cfg.ReNominal = value; OnPropertyChanged(); EnsureInputMeterCt(forceRatio: true); Recalculate(); } } }
    public string ReManufacturer { get => _cfg.ReManufacturer; set { if (_cfg.ReManufacturer != value) { _cfg.ReManufacturer = value; OnPropertyChanged(); OnPropertyChanged(nameof(ReNominals)); Recalculate(); } } }
    public bool AvInOn { get => _cfg.AvInOn; set { if (_cfg.AvInOn != value) { _cfg.AvInOn = value; OnPropertyChanged(); Recalculate(); } } }
    public int AvInNominal { get => _cfg.AvInNominal; set { if (_cfg.AvInNominal != value) { DisableAutoSelectionForManualEdit(); _cfg.AvInNominal = value; OnPropertyChanged(); EnsureInputMeterCt(forceRatio: true); Recalculate(); } } }
    public string AvInManufacturer { get => _cfg.AvInManufacturer; set { if (_cfg.AvInManufacturer != value) { _cfg.AvInManufacturer = value; OnPropertyChanged(); OnPropertyChanged(nameof(AvInNominals)); Recalculate(); } } }
    ...
    public string CtRatio { get => _cfg.CtRatio; set { if (_cfg.CtRatio != value) { DisableAutoSelectionForManualEdit(); _cfg.CtRatio = value; OnPropertyChanged(); Recalculate(); } } }
    public string CtAccuracyClass { get => _cfg.CtAccuracyClass; set { if (_cfg.CtAccuracyClass != value) { _cfg.CtAccuracyClass = value; OnPropertyChanged(); Recalculate(); } } }
    public bool HasCtKip { get => _cfg.HasCtKip; set { if (_cfg.HasCtKip != value) { _cfg.HasCtKip = value; OnPropertyChanged(); Recalculate(); } } }
    public string CtKipRatio { get => _cfg.CtKipRatio; set { if (_cfg.CtKipRatio != value) { DisableAutoSelectionForManualEdit(); _cfg.CtKipRatio = value; OnPropertyChanged(); Recalculate(); } } }
    public string CtKipAccuracyClass { get => _cfg.CtKipAccuracyClass; set { if (_cfg.CtKipAccuracyClass != value) { _cfg.CtKipAccuracyClass = value; OnPropertyChanged(); Recalculate(); } } }
    ...
    public bool HasRise
    {
        ...
        set
        {
            ...
            OnPropertyChanged(nameof(ShowNhRiseWarning));
            Recalculate();
        }
    }
    ...
    private AuxiliaryNeedsConfig CreateDefaultAuxiliaryNeeds()
    {
        ...
        var branchBreakerModel = DefaultAuxBranchBreakerModel(auxBreakerManufacturer);
        ...
        return new AuxiliaryNeedsConfig
        {
            ...
            SocketBreakerModel = branchBreakerModel,
            SocketBreakerNominal = DefaultDeviceNominal("АВ", auxBreakerManufacturer, branchBreakerModel, 16, "ЩСН"),
            SocketModel = DefaultDeviceModel("розетка", "", "ЩСН"),
            SocketQuantity = 1,
            HeatingEnabled = false,
            HeatingBreakerManufacturer = auxBreakerManufacturer,
            HeatingBreakerModel = branchBreakerModel,
            HeatingBreakerNominal = DefaultDeviceNominal("АВ", auxBreakerManufacturer, branchBreakerModel, 10, "ЩСН"),
            ...
            VentilationBreakerModel = branchBreakerModel,
            VentilationBreakerNominal = DefaultDeviceNominal("АВ", auxBreakerManufacturer, branchBreakerModel, 6, "ЩСН"),
            ...
            RieseProtectionModel = branchBreakerModel,
    ...
    private IReadOnlyList<int> DeviceNominals(string type, string manufacturer, string model, string installationArea = "", string purpose = "")
    {
        ...
        return dm is not null && dm.Nominals.Count > 0
            ? dm.Nominals.OrderBy(value => value).ToList()
            : Array.Empty<int>();
    }
    ...
    private string DefaultAuxBranchBreakerModel(string manufacturer) =>
        DeviceModelsFor("АВ", "ЩСН")
            .Where(model => model.Manufacturer.Equals(manufacturer, StringComparison.OrdinalIgnoreCase))
            .OrderBy(model => model.Poles == 1 ? 0 : 1)
            .ThenBy(model => model.Nominals.DefaultIfEmpty(int.MaxValue).Min())
            .Select(CatalogModelName)
            .FirstOrDefault() ?? "";
    ...
    private int NormalizeDeviceNominal(
        string type,
        string manufacturer,
        string model,
        int current,
        ...
        string purpose = "")
    {
        var values = DeviceNominals(type, manufacturer, model, installationArea, purpose);
        if (values.Contains(current))
            return current;
        return values.Where(value => value >= fallback).DefaultIfEmpty(0).Min();
    }
    ...
    private void SetAuxString(string propertyName, string value, Action<string> assign, Action? after = null)
    {
        ...
        if (propertyName switch
        {
            ...
            nameof(OpsType) => Aux.OpsType == normalized,
            nameof(OpsModel) => Aux.OpsModel == normalized,
    ...
    private void SetAuxInt(string propertyName, int value, Action<int> assign)
    {
        var current = propertyName switch
        {
            ...
            nameof(OpsLoops) => Aux.OpsLoops,
            nameof(RiesePowerVa) => Aux.RiesePowerVa,
            nameof(RieseAutonomyMinutes) => Aux.RieseAutonomyMinutes,
            _ => throw new ArgumentOutOfRangeException(nameof(propertyName), propertyName, "Неизвестное числовое поле собственных нужд."),
        };
    ...
    {
        Aux.MainBreakerNominal = NormalizeDeviceNominal(
            "АВ", Aux.MainBreakerManufacturer, Aux.MainBreakerModel, Aux.MainBreakerNominal, 25, "ЩСН", "Ввод ЩСН");
    ...
    {
        Aux.LightingBreakerNominal = NormalizeDeviceNominal(
            "АВ", Aux.LightingBreakerManufacturer, Aux.LightingBreakerModel, Aux.LightingBreakerNominal, 10, "ЩСН", "Цепь освещения");
    ...
    {
        Aux.SocketBreakerModel = DefaultAuxBranchBreakerModel(Aux.SocketBreakerManufacturer);
    ...
    {
        Aux.SocketBreakerNominal = NormalizeDeviceNominal(
            "АВ", Aux.SocketBreakerManufacturer, Aux.SocketBreakerModel, Aux.SocketBreakerNominal, 16, "ЩСН");
    ...
    {
        Aux.HeatingBreakerModel = DefaultAuxBranchBreakerModel(Aux.HeatingBreakerManufacturer);
    ...
    {
        Aux.HeatingBreakerNominal = NormalizeDeviceNominal(
            "АВ", Aux.HeatingBreakerManufacturer, Aux.HeatingBreakerModel, Aux.HeatingBreakerNominal, 10, "ЩСН");
    ...
    {
        Aux.VentilationBreakerModel = DefaultAuxBranchBreakerModel(Aux.VentilationBreakerManufacturer);
    ...
    {
        Aux.VentilationBreakerNominal = NormalizeDeviceNominal(
            "АВ", Aux.VentilationBreakerManufacturer, Aux.VentilationBreakerModel, Aux.VentilationBreakerNominal, 6, "ЩСН");
    ...
    {
        Aux.RieseProtectionModel = DefaultAuxBranchBreakerModel(Aux.RieseProtectionManufacturer);
    ...
    private void EnsureInputMeterCt(bool forceRatio)
    {
        ...
        {
            _cfg.CtRatio = SuggestedCtRatio(ElectricalSelectionService.CurrentTransformerReferenceNominal(_cfg));
    ...
    private int InputNominalFromConfig() => ElectricalSelectionService.LimitingInputNominal(_cfg);
    ...
    public IReadOnlyList<string> ModelsForFeeder(string deviceType, string manufacturer)
    {
        var models = _env.Catalog.DeviceModels
            .Where(a => a.Type == deviceType
                && a.Manufacturer == manufacturer
                && IsRunnOutgoingModel(a))
            .Select(CatalogModelName)
            .Where(s => !string.IsNullOrWhiteSpace(s))
            .Distinct(StringComparer.OrdinalIgnoreCase)
            .ToList();
    ...
    public IReadOnlyList<int> NominalsForFeeder(string deviceType, string manufacturer, string model)
    {
        ...
        IReadOnlyList<int> nominals = dm != null && dm.Nominals.Count > 0
            ? dm.Nominals
            : Array.Empty<int>();
        if (!deviceType.Equals("АВ", StringComparison.OrdinalIgnoreCase))
            return nominals;

        var inputNominal = InputNominalFromConfig();
        return inputNominal > 0
            ? nominals.Where(value => value <= inputNominal).ToList()
            : nominals;
    }

    internal int NormalizeFeederNominal(string deviceType, string manufacturer, string model, int current)
    {
        var values = NominalsForFeeder(deviceType, manufacturer, model);
        if (values.Contains(current))
            return current;
        return values.Where(value => value <= current).DefaultIfEmpty(values.FirstOrDefault()).Max();
    }
    ...
    private static bool IsRunnOutgoingModel(DeviceModel model) =>
    ...
    {
        return ElectricalSelectionService.RecommendedCtRatio(TtRatios, nominal);
    }
    ...
    internal void DisableAutoSelectionForManualEdit()
    {
        if (!_cfg.AutoElectricalSelection)
            return;
        _cfg.AutoElectricalSelection = false;
        OnPropertyChanged(nameof(AutoElectricalSelection));
        OnPropertyChanged(nameof(AutoSelectionSummary));
    }
    ...
    public string MassBreakdownSummary => ShowDoubleKtpnConfiguration
        ? $"Секция 1 {_res.Section1EstimatedMass:0} кг; секция 2 {_res.Section2EstimatedMass:0} кг; базовая масса {_res.GrossMass:0} кг"
        : ShowKtpnTabs
        ? $"ТМГ {_res.TransformerMass:0} кг + основание {_res.BaseMass:0} кг + корпус {_res.BodyMass:0} кг = {_res.GrossMass:0} кг"
        : $"Сумма масс панелей/ячеек и оболочек: {_res.GrossMass:0} кг";
    ...
    public string MassBasisSummary => !ShowKtpnTabs
        ? "Основа массы: сумма ориентировочных масс панелей или ячеек; уточнить по рабочей компоновке"
        : _res.UsesManualMass
            ? "Основа массы: паспортная масса трансформаторов + расчет корпуса с ручными уточнениями"
            : "Основа массы: паспортная масса трансформаторов + расчет основания и корпуса";
    public string MassMethodologySummary
    {
        get
        {
            if (!ShowKtpnTabs)
                return "Габарит по линейке шкафов; масса и шины предварительные до тепловой, электродинамической и компоновочной проверки.";
    ...
    public string TransformerResultSummary => ShowDoubleKtpnConfiguration
        ? $"Т1: {ValueOrDash(_cfg.Mark)}, {_res.Section1RatedCurrentA:0} А; Т2: {ValueOrDash(_cfg.DoubleKtpn.SecondTransformerMark)}, {_res.Section2RatedCurrentA:0} А"
        : ShowKtpnTabs
            ? Tr is null ? "ТМГ не выбран" : $"{Tr.PowerKva:0} кВА; ток ТМГ {_res.RatedCurrentA:0} А"
            : SelectedProduct.DisplayName;
    public string InputResultSummary => ShowDoubleKtpnConfiguration
        ? $"вводы {_cfg.DoubleKtpn.Section1InputNominalA}/{_cfg.DoubleKtpn.Section2InputNominalA} А; секционный {_cfg.DoubleKtpn.SectionCouplerNominalA} А; КЗ секций {_res.Section1ShortCircuitCurrentKa:0.#}/{_res.Section2ShortCircuitCurrentKa:0.#} кА"
        : ShowLowVoltageConfiguration
            ? $"{_cfg.LowVoltageAssembly.Series}; {_cfg.LowVoltageAssembly.LineupTemplate}; {_cfg.LowVoltageAssembly.RatedVoltageV} В; шины {_cfg.LowVoltageAssembly.RatedBusCurrentA} А; КЗ {_cfg.LowVoltageAssembly.DesignShortCircuitCurrentKa:0.#} кА"
            : ShowMediumVoltageConfiguration
                ? $"{_cfg.MediumVoltageSwitchgear.Series}; {_cfg.MediumVoltageSwitchgear.LineupTemplate}; {_cfg.MediumVoltageSwitchgear.RatedVoltageKv:0.#} кВ; шины {_cfg.MediumVoltageSwitchgear.RatedBusCurrentA} А; отключение {_cfg.MediumVoltageSwitchgear.BreakerBreakingCurrentKa:0.#} кА"
                : _res.InputNominal > 0 ? $"{DocumentBuilder.InputDeviceDescription(_cfg)}; ограничивающий номинал {_res.InputNominal:0} А" : "не задан";
    public string DoubleKtpnSectionSummary =>
        $"Секция 1: ТМГ {_res.Section1RatedCurrentA:0} А, ввод {_res.Section1InputNominalA:0} А, КЗ {_res.Section1ShortCircuitCurrentKa:0.#} кА, шины {_res.Section1BusbarLv}, масса {_res.Section1EstimatedMass:0} кг\n"
        + $"Секция 2: ТМГ {_res.Section2RatedCurrentA:0} А, ввод {_res.Section2InputNominalA:0} А, КЗ {_res.Section2ShortCircuitCurrentKa:0.#} кА, шины {_res.Section2BusbarLv}, масса {_res.Section2EstimatedMass:0} кг\n"
        + $"Проверка отключающей способности: вводы и секционный аппарат должны быть не ниже максимального КЗ секций {Math.Max(_res.Section1ShortCircuitCurrentKa, _res.Section2ShortCircuitCurrentKa):0.#} кА.";
    public string FeedersResultSummary
    {
        get
        {
            if (ShowLowVoltageConfiguration)
                return $"{_cfg.LowVoltageAssembly.Panels.Count} шт.; секций {_cfg.LowVoltageAssembly.SectionCount}";
            if (ShowMediumVoltageConfiguration)
                return $"{_cfg.MediumVoltageSwitchgear.Cells.Count} шт.";
    ...
    public string BusbarResultSummary => ShowDoubleKtpnConfiguration
        ? $"РУВН {BusbarDescription(_cfg.BusbarHvMaterial, _res.BusbarHv)}; РУНН С1 {BusbarDescription(_cfg.BusbarLvMaterial, _res.Section1BusbarLv)}; РУНН С2 {BusbarDescription(_cfg.BusbarLvMaterial, _res.Section2BusbarLv)}; N С1/С2 {BusbarDescription(_cfg.BusbarNMaterial, _res.Section1BusbarN)} / {BusbarDescription(_cfg.BusbarNMaterial, _res.Section2BusbarN)}; PE {_res.Section1BusbarPe}/{_res.Section2BusbarPe}"
        : ShowLowVoltageConfiguration
        ? $"Сборные шины {_res.BusbarLv}; N {_res.BusbarN}; PE {_res.BusbarPe}"
        : ShowMediumVoltageConfiguration
            ? $"Сборные шины {_res.BusbarHv}"
    ...
    public string EquipmentTrustSummary
    {
        ...
        {
            if (ShowLowVoltageConfiguration)
                return LineupTrustSummary("панелей", _cfg.LowVoltageAssembly.Panels.Select(panel => panel.EquipmentSourceConfidence));
            if (ShowMediumVoltageConfiguration)
                return LineupTrustSummary("ячеек", _cfg.MediumVoltageSwitchgear.Cells.Select(cell => cell.EquipmentSourceConfidence));
    ...
    private static string LineupTrustSummary(string itemName, IEnumerable<string> confidences)
    {
        ...
        return $"{itemName}: {values.Count}; проверено: {verified}; требуется проверка: {needsVerification}; задано проектом: {userInput}";
    }
    ...
    public string StatusColor => _res.HasErrors
        ? (_cfg.ErrorsAcceptedForWork ? "#FB8C00" : "#E53935")
        : (ValidationMessages.Any(m => m.Severity == Severity.Warning) ? "#FB8C00" : "#00A152");
    ...
    public void Recalculate()
    {
        ...
        UpdateAutoSelectionState(autoChanges);
        _res = CalculationEngine.Calculate(_cfg, _env.Catalog);
        ValidationMessageClassifier.Apply(_res.Messages);

        ValidationMessages.Clear();
        foreach (var msg in _res.Messages)
            ValidationMessages.Add(msg);
        CustomerRequirementMessages.Clear();
        foreach (var msg in (ShowKtpnTabs ? CustomerRequirementEngine.Build(_cfg, _env.Catalog) : Array.Empty<ValidationMessage>()))
        ...
        {
            var package = BuildDocuments();
            var order = package.Single(document => document.Kind == GeneratedDocumentKind.ProductionOrder);
            var passport = package.Single(document => document.Kind == GeneratedDocumentKind.Passport);
            var checklist = package.Single(document => document.Kind == GeneratedDocumentKind.Checklist);
            var specification = package.Single(document => document.Kind == GeneratedDocumentKind.Specification);
    ...
    private void RaiseOutputs()
    {
        foreach (var name in new[]
        {
            ...
            nameof(MassMethodologySummary), nameof(MassScopeWarning), nameof(MassBasisSummary),
            ...
            nameof(InputResultSummary), nameof(DoubleKtpnSectionSummary), nameof(FeedersResultSummary), nameof(BusbarResultSummary),
            nameof(PvrNominals), nameof(ReNominals), nameof(AvInNominals),
            ...
            nameof(ProjectVersionText), nameof(ProjectVersionNumberText), nameof(AutoElectricalSelection), nameof(AutoSelectionSummary),
            nameof(EquipmentDatabaseSummary), nameof(EquipmentDatabaseDetails),
            nameof(ProjectTabStatusColor), nameof(TransformerTabStatusColor), nameof(RuvnTabStatusColor),
            nameof(RunnTabStatusColor), nameof(AuxiliaryTabStatusColor), nameof(ProductTabStatusColor), nameof(ResultTabStatusColor), nameof(DocumentsTabStatusColor),
            nameof(ProjectTabStatusText), nameof(TransformerTabStatusText), nameof(RuvnTabStatusText),
            nameof(RunnTabStatusText), nameof(AuxiliaryTabStatusText), nameof(ProductTabStatusText), nameof(ResultTabStatusText), nameof(DocumentsTabStatusText),
            nameof(ValidationGroups), nameof(HasRecoveryProject),
            nameof(IsDirty), nameof(HasUnsavedChanges), nameof(UnsavedChangesText), nameof(CurrentProjectFileText),
            nameof(IsExporting), nameof(ExportProgressText), nameof(WindowTitle),
            nameof(PassportPreview), nameof(OrderPreview), nameof(ChecklistPreview), nameof(SpecificationPreview),
            nameof(SelectedProduct), nameof(ApplicationTitle), nameof(ProductConfigurationHeader), nameof(ProductDimensionsTitle),
            nameof(ProductPassportTabTitle), nameof(ProductPrimaryResultLabel), nameof(ProductInputResultLabel), nameof(ProductLineupResultLabel),
            nameof(ShowKtpnTabs), nameof(ShowProductConfigurationTab),
            nameof(ShowDoubleKtpnConfiguration), nameof(ShowLowVoltageConfiguration), nameof(ShowMediumVoltageConfiguration),
            nameof(LvPanels), nameof(MvCells),
        })
            OnPropertyChanged(name);

        ExportExcelCommand.RaiseCanExecuteChanged();
        ExportPdfCommand.RaiseCanExecuteChanged();
        RestoreRecoveryCommand.RaiseCanExecuteChanged();
    ...
    {
        ApplyConfig(CreateDefaultConfig(), applyGridProfile: true, normalizeMeterCt: false);
    ...
    /// <summary>
    /// Подменяет конфигурацию проекта и перестраивает все зависимые коллекции.
    /// Флаг подавления пересчёта снимается в finally: исключение на середине
    /// не должно оставлять приложение с навсегда выключенным пересчётом.
    /// </summary>
    private void ApplyConfig(ProjectConfig cfg, bool applyGridProfile, bool normalizeMeterCt)
    {
        SuspendProjectTracking();
        _suspendRecalc = true;
        try
        {
            _cfg = cfg;
    ...
    private void OpenProject()
    {
        ...
        try
        {
            loaded = ProjectStorage.Load(path);
        }
        catch (Exception ex)
        {
            Notify?.Invoke($"Не удалось открыть проект «{path}»: {ex.Message}");
            return;
        }
        ApplyConfig(loaded, applyGridProfile: false, normalizeMeterCt: true);
        SetCurrentProjectPath(path);
        EnableProjectTracking(markDirty: false);
        Notify?.Invoke($"Проект загружен: {path}");
    }

    private bool CanExportDocuments() => CanReleaseDocuments && !IsExporting;
    ...
    private async void ExportExcel()
    {
        ...
        if (string.IsNullOrEmpty(path)) return;
        await ExportInBackground(path, "Excel", (snapshot, result, documents) =>
            ExcelExporter.Export(path, snapshot, result, documents, _env.Catalog));
    }
    ...
    private async void ExportPdf()
    {
        ...
        if (string.IsNullOrEmpty(path)) return;
        await ExportInBackground(path, "PDF", (snapshot, result, documents) =>
            PdfExporter.Export(path, documents, snapshot, result, _env.Catalog));
    }
...
public sealed class OutgoingFeederViewModel : ObservableObject
{
    ...
    public OutgoingFeederViewModel(MainViewModel owner, OutgoingFeederConfig model)
    {
        _owner = owner;
        _model = model;
        DuplicateCommand = new RelayCommand(() => _owner.DuplicateFeeder(this), () => _owner.CanDuplicateFeeder(this));
        DeleteCommand = new RelayCommand(() => _owner.DeleteFeeder(this));
        NormalizeMetering();
    }

    internal OutgoingFeederConfig Config => _model;
    ...
    public bool ShowSectionSelector => _owner.ShowDoubleKtpnConfiguration;
    public IReadOnlyList<int> SectionOptions => _owner.DoubleKtpnSectionOptions;
    public int SectionNumber
    {
        get => _model.SectionNumber;
        set
        {
            var normalized = Math.Clamp(value, 1, 2);
            if (_model.SectionNumber == normalized) return;
            _model.SectionNumber = normalized;
            OnPropertyChanged();
    ...
    public IReadOnlyList<int> Nominals
    {
        get
        {
            var values = _owner.NominalsForFeeder(_model.DeviceType, _model.Manufacturer, _model.Model).ToList();
            if (_model.Nominal > 0 && !values.Contains(_model.Nominal))
                values.Insert(0, _model.Nominal);
            return values;
        }
    }
    public IReadOnlyList<string> TtRatios => _owner.TtRatios;
    ...
    public string Manufacturer
    {
        ...
        set
        {
            ...
            NormalizeNominalAfterModelChange();
            OnPropertyChanged();
    ...
    public string TtRatio
    {
        ...
        set
        {
            if (_model.TtRatio == value)
                return;
            _owner.DisableAutoSelectionForManualEdit();
    ...
    private void NormalizeNominalAfterModelChange()
    {
        var normalized = _owner.NormalizeFeederNominal(
            _model.DeviceType, _model.Manufacturer, _model.Model, _model.Nominal);
        if (normalized > 0)
            _model.Nominal = normalized;
        if (_model.HasMeter)
            _model.TtRatio = _owner.SuggestedCtRatio(_model.Nominal);
    ...
    internal void RefreshElectricalSelection()
    {
        OnPropertyChanged(nameof(Nominal));
        OnPropertyChanged(nameof(Nominals));
        OnPropertyChanged(nameof(TtRatio));
