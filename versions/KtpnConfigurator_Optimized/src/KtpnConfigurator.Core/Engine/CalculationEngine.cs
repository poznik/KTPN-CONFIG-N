...
public static class CalculationEngine
{
    ...
    {
        // Пустой/неизвестный тип изделия считаем одиночной КТПН (историческое поведение).
        var isSingleKtpn = string.IsNullOrWhiteSpace(c.ProductTypeId)
            || c.ProductTypeId.Equals(ProductTypeIds.SingleKtpn, StringComparison.OrdinalIgnoreCase);
        if (!isSingleKtpn)
        {
            // Расчёт не мутирует проект: только починка null-вложений, без
            // повторного применения шаблонов (иначе удалённые панели «воскресали»).
            ProductConfigurationDefaults.EnsureNestedConfigs(c);
            return ProductCalculationEngine.Calculate(c, store);
        }
        ...
        res.UsesManualMass = c.ManualBaseMass.HasValue || c.ManualBodyMass.HasValue || c.ManualGrossMass.HasValue;
        ...
        res.InputNominal = ElectricalSelectionService.LimitingInputNominal(c);
    ...
    internal static string SelectTransformerBusbar(string tableSection, string material)
    {
    ...
    internal static string NormalizeBusbarSection(string section)
    {
