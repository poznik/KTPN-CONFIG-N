...
public static partial class ValidationEngine
{
    public static void Apply(ProjectConfig c, CalculationResult res, CatalogStore store, bool transformerFound)
    {
        ...
        CheckInputNominalsAgainstCatalog(c, store, msgs);
        CheckAutomaticInputAvailability(c, res.RatedCurrentA, store, msgs);
        CheckMetering(c, msgs);
        var ctReference = ElectricalSelectionService.CurrentTransformerReferenceNominal(c);
        CheckCurrentTransformerRatio(store, "ТТ учета на вводе", c.CtRatio, ctReference, msgs, enabled: c.HasCt);
        CheckCurrentTransformerRatio(store, "ТТ КИП на вводе", c.CtKipRatio, ctReference, msgs, enabled: c.HasCtKip);
        CheckRuvn(c, store, msgs);
        CheckOutgoingFeeders(c, res.InputNominal, store, msgs);
        CheckAuxiliaryNeeds(c, store, msgs);
        CheckNhRiseRecommendation(c, msgs);
    ...
    private static void CheckAutomaticInputAvailability(
        ...
        List<ValidationMessage> messages)
    {
        ...
        foreach (var device in ProjectConfigNormalizer.GetLvInputDevices(config))
        {
            var recommended = ElectricalSelectionService.RecommendedInputNominal(
                device.DeviceType, device.Manufacturer, transformerCurrent, store);
            if (recommended > 0)
                continue;

            messages.Add(new(Severity.Error,
                $"РУНН: для {device.Caption}, производитель {device.Manufacturer}, в базе нет номинала не ниже тока ТМГ {transformerCurrent:0} А. Выберите другой тип или производителя аппарата."));
        }
    }

    private static void CheckInputNominalsAgainstCatalog(
        ProjectConfig config,
        CatalogStore store,
        List<ValidationMessage> messages)
    {
        foreach (var device in ProjectConfigNormalizer.GetLvInputDevices(config))
        {
            var available = ElectricalSelectionService.InputNominals(device.DeviceType, device.Manufacturer, store);
            if (available.Contains(device.Nominal))
                continue;

            messages.Add(new(Severity.Error,
                $"РУНН: номинал {device.Nominal} А отсутствует в базе для {device.Caption}, производитель {device.Manufacturer}."));
        }
    }
    ...
    private static void CheckMetering(ProjectConfig c, List<ValidationMessage> msgs)
    {
        ...
        if (c.HasCt && string.IsNullOrWhiteSpace(c.CtRatio))
            msgs.Add(new(Severity.Error, "Включены ТТ учета на вводе РУНН, но не задан коэффициент трансформации."));
        if (c.HasCt && string.IsNullOrWhiteSpace(c.CtAccuracyClass))
            msgs.Add(new(Severity.Error, "Включены ТТ учета на вводе РУНН, но не выбран класс точности."));
        if (c.HasCtKip && string.IsNullOrWhiteSpace(c.CtKipRatio))
            msgs.Add(new(Severity.Error, "Включены ТТ КИП на вводе РУНН, но не задан коэффициент трансформации."));
        if (c.HasCtKip && string.IsNullOrWhiteSpace(c.CtKipAccuracyClass))
            msgs.Add(new(Severity.Error, "Включены ТТ КИП на вводе РУНН, но не выбран класс точности."));
    ...
    private static void CheckNhRiseRecommendation(ProjectConfig config, List<ValidationMessage> messages)
    {
        if (config.PvrOn && config.AuxiliaryNeeds?.HasRise != true)
        {
            messages.Add(new(Severity.Warning,
                "При выборе NH обычно предусматривают РИСЭ. Проверьте необходимость РИСЭ в собственных нуждах."));
        }
    }
    ...
    private static void CheckOutgoingFeeders(ProjectConfig c, double inputNominal, CatalogStore store, List<ValidationMessage> msgs)
    {
        foreach (var feeder in c.OutgoingFeeders ?? new List<OutgoingFeederConfig>())
        {
            ...
            var model = FindFeederDeviceModel(store, feeder.DeviceType, feeder.Manufacturer, feeder.Model);
            if (model is null)
                msgs.Add(new(Severity.Error, $"{title}: выбранная модель {feeder.Manufacturer} {feeder.Model} отсутствует в базе оборудования."));
            if (model is not null && model.Nominals.Count > 0 && !model.Nominals.Contains(feeder.Nominal))
                msgs.Add(new(Severity.Error, $"{title}: номинал {feeder.Nominal} А отсутствует в базе модели {feeder.Model}."));
            if (model is not null && HasFeederMetering(feeder) && !model.CompatibleWithMeter)
                msgs.Add(new(Severity.Warning, $"{title}: для модели {feeder.Model} в базе не подтверждена совместимость с учетом."));
            if (model is not null
                && !string.IsNullOrWhiteSpace(model.InstallationArea)
                && !model.InstallationArea.Contains("РУНН", StringComparison.OrdinalIgnoreCase))
            {
                msgs.Add(new(Severity.Warning, $"{title}: место установки модели {feeder.Model} не соответствует РУНН."));
            }
            if (model is not null
                && !string.IsNullOrWhiteSpace(model.Voltage)
                && !model.Voltage.Contains("0,4", StringComparison.OrdinalIgnoreCase)
                && !model.Voltage.Contains("400", StringComparison.OrdinalIgnoreCase))
            {
                msgs.Add(new(Severity.Error, $"{title}: напряжение модели {feeder.Model} не соответствует РУНН 0,4 кВ."));
            }
    ...
    private static void CheckModelNominal(CatalogStore store, string type, string manufacturer, string model, int nominal, string label, List<ValidationMessage> msgs)
    {
        ...
        if (dm is null)
            msgs.Add(new(Severity.Error, $"{label}: модель {manufacturer} {model} отсутствует в базе оборудования."));
        if (dm is not null && dm.Nominals.Count > 0 && nominal > 0 && !dm.Nominals.Contains(nominal))
            msgs.Add(new(Severity.Error, $"{label}: номинал {nominal} А отсутствует в базе модели {model}."));
    ...
    private static DeviceModel? FindFeederDeviceModel(CatalogStore store, string type, string manufacturer, string model)
    {
        return store.DeviceModels.FirstOrDefault(d =>
            d.Type.Equals(type, StringComparison.OrdinalIgnoreCase)
            ...
            && (d.Model.Equals(model, StringComparison.OrdinalIgnoreCase)
                || d.Series.Equals(model, StringComparison.OrdinalIgnoreCase))
            && (d.InstallationArea.Contains("РУНН", StringComparison.OrdinalIgnoreCase)
                || d.ApplicationRange.Contains("РУНН", StringComparison.OrdinalIgnoreCase))
            && (d.Purpose.Contains("отход", StringComparison.OrdinalIgnoreCase)
                || d.ApplicationRange.Contains("отход", StringComparison.OrdinalIgnoreCase)));
    }
        ...
        bool enabled = true)
    {
        ...
        var recommended = ElectricalSelectionService.RecommendedCtRatio(store.Options.TtRatios, nominal);
        if (!string.IsNullOrWhiteSpace(recommended)
    ...
    private static bool IsBusBridgeInput(ProjectConfig c) =>
        c.RuvnExecution?.Contains("шинн", StringComparison.OrdinalIgnoreCase) == true;
    ...
    /// <summary>Парсит диапазон вида "16 - 4000 А" или "31,5 - 4000 А".</summary>
    private static (double min, double max) ParseRange(string? range)
    {
        ...
        // Десятичный разделитель в каталоге бывает и точкой, и запятой.
        if (nums.Count >= 2
            && double.TryParse(nums[0].Value.Replace(',', '.'), NumberStyles.Float, CultureInfo.InvariantCulture, out var a)
            && double.TryParse(nums[1].Value.Replace(',', '.'), NumberStyles.Float, CultureInfo.InvariantCulture, out var b))
            return (a, b);
        return (0, 0);
    }
