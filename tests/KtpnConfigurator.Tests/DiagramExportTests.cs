...
public class DiagramExportTests
{
    ...
    [Fact]
    public void ExcelExport_AddsDiagramSheet()
    {
        RunSta(() =>
        {
            ...
            try
            {
                ...
                var sheet = workbook.Worksheet("Схема");
                Assert.NotNull(sheet);
                Assert.True(sheet.Pictures.Count() > 0);

                var metering = workbook.Worksheet("Цепи учета");
                Assert.NotNull(metering);
                Assert.True(metering.Pictures.Count() > 0);
            }
            finally
            {
                TryDelete(path);
    ...
    [Fact]
    public void PdfExport_WithDiagram_IsLargerThanDocumentsOnly()
    {
        RunSta(() =>
        {
            var (cfg, res, docs, store) = BuildSampleProject();
            ...
            try
            {
                ...
                Assert.True(withDiagramSize > docsOnlySize + 20_000);
            }
            finally
            {
