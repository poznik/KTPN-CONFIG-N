...
public static class SpecificationBuilder
{
    ...
    public static IReadOnlyList<SpecificationItem> GenerateSpecification(ProjectConfig config, CalculationResult result, CatalogStore store)
    {
        ...
        if (config.HasCt)
            Add(items, ref pos, PowerSection, "TA1-TA3", "Трансформаторы тока учета", "ТТ", "", ValueOrDash(config.CtRatio),
                3, "шт", "project", "userInput");
        if (config.HasCtKip)
            Add(items, ref pos, PowerSection, "TAK1-TAK3", "Трансформаторы тока КИП", "ТТ КИП", "", ValueOrDash(config.CtKipRatio),
                3, "шт", "project", "userInput");
        if (config.HasMeter)
