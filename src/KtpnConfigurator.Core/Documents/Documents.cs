...
public static class DocumentBuilder
{
    ...
    public static GeneratedDocument BuildProductionOrder(ProjectConfig c, CalculationResult res, CatalogStore store, DocTemplates tpl)
    {
        ...
        doc.Sections.Add(new DocSection
        {
            ...
            Rows =
            {
                new("Заводской номер / дата", "№ ________ / __.__.202_ г.", "подпись"),
    ...
    {
        if (c.PvrOn) return $"ПВР {CurrentA(c.PvrNominal)} ({c.PvrManufacturer})";
        if (c.ReOn) return $"Рубильник РЕ {CurrentA(c.ReNominal)} ({c.ReManufacturer})";
        if (c.AvInOn) return $"Вводной АВ {CurrentA(c.AvInNominal)} ({c.AvInManufacturer})";
    ...
    private static string LvEquipmentDescription(ProjectConfig c)
    {
        ...
        if (c.HasCt)
            parts.Add($"ТТ {ValueOrDash(c.CtRatio)}");
        if (c.HasCtKip)
            parts.Add($"ТТ КИП {ValueOrDash(c.CtKipRatio)}");
        if (c.HasMeter)
    ...
    private static string EnclosureDetailsDescription(ProjectConfig c) =>
        string.Join("; ", new[]
        {
            ...
            c.HasServicePlatform ? "площадка обслуживания" : "",
