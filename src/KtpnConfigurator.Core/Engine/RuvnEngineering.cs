...
public static partial class RuvnEngineering
{
    ...
    public static bool HasRuvn(ProjectConfig config) =>
        !config.RuvnType.Equals(NoRuvn, StringComparison.OrdinalIgnoreCase);

    public static bool IsPassThrough(ProjectConfig config) =>
        config.RuvnType.Equals(PassThroughRuvn, StringComparison.OrdinalIgnoreCase);
    ...
    private static RuvnBranchSelection TransformerBranch(ProjectConfig config)
    {
        ...
        var fuseOn = config.RuvnTransformerFuseOn || HasDevice(fuseType) || HasDevice(fuseNominal);
