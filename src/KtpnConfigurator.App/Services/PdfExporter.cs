...
public static class PdfExporter
{
    public static void Export(string path, IEnumerable<GeneratedDocument> docs, ProjectConfig? cfg = null, CalculationResult? res = null, CatalogStore? catalog = null)
    {
        ...
        Document.Create(container =>
        {
            foreach (var doc in docs)
            {
                container.Page(page =>
                {
                    page.Size(doc.Kind == GeneratedDocumentKind.Checklist ? PageSizes.A4.Landscape() : PageSizes.A4);
                    page.Margin(28);
            ...
            if (cfg is not null && res is not null)
                RenderDiagramPage(container, cfg, res, catalog!);
        }).GeneratePdf(path);
    }
        ...
        container.Column(col =>
        {
            ...
            foreach (var sec in doc.Sections)
            {
                col.Item().Table(table =>
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(sec.Name))
                        table.Cell().ColumnSpan(3).Element(SectionCell).Text(sec.Name).Bold();
    ...
    private static void RenderChecklist(IContainer container, GeneratedDocument doc)
    {
        container.Table(table =>
        {
            ...
            {
                if (!string.IsNullOrWhiteSpace(sec.Name))
                    table.Cell().ColumnSpan(5).Element(SectionCell).Text(sec.Name).Bold();
